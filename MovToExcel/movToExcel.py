...
class MovToExcel(QtGui.QDialog, Ui_MovToExcel):
	...
	def __init__(self, parent=None):
		...
		self.job_thread = threading.Thread(target=self.to_excel_btn_clicked)
	...
	def to_excel_btn_clicked(self):
		...
		# 获取所有镜头列表
		video_file_list = []
		for root_name, dir_name, file_name in os.walk(self.path_line.text()):
			if dir_name == []:
		...
		# 设置进度条最大值
		self.progressBar.setMaximum(len(video_file_list))
		self.excel_sig.connect(self.progressBar.setValue)
		...
		for index, i in enumerate(video_file_list):
			...
			# 设置当前进度条
			self.excel_sig.emit(index + 1)

		# shot_list返回值
		# [['S05_06bu_fx_00100', 119], ['S05_06bu_fx_00200', 50], ['S05_06bu_fx_00201', 50]]

		# 生成表格
		workbook = xlsxwriter.Workbook(self.excel_path_line.text())
		...
		self.close()
		os.startfile("/".join(self.excel_path_line.text().split("/")[:-1]))
